...
draw_function(void* thread_num)
{
    ...
    /* for every pixel */
    for(int px = *((int*)thread_num); px < width; px += nthreads)
    {
        ...
        for( int py=0; py<height; ++py )
        {
             ...
             */
            Color curr;
            curr.r = scaled_color[0];
            curr.g = scaled_color[1];
            curr.b = scaled_color[2];
            colors[px][py] = curr; 
        }
    }
...
main( int argc, char **argv )
{
    ...
    //we doing the multithread now
    pthread_t* threads = (pthread_t*)malloc(sizeof(pthread_t) * nthreads);
    int* pids = (int*)malloc(sizeof(int) * nthreads);
    if(threads == NULL || pids == NULL)
    {
    ...
    {
        pids[i] = i;
        int iret = pthread_create(&threads[i], NULL, draw_function, &pids[i]);
        if(iret)
        {
